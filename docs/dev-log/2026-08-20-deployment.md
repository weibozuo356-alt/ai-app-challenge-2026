...
- 将网页语言、标题、描述和分享信息改为 BugMentor 正式内容
- 为前后端增加 CSP、防 iframe、MIME 嗅探防护和权限策略
- 为 AI 请求增加 30 秒超时，为 React 首屏增加错误兜底页
- 限制不可信请求 ID，避免异常内容污染日志
- 增加隐私、安全、更新日志和部署后自动冒烟验收脚本
- 增加 Dependabot 每周依赖更新检查
- 前端生产构建：通过
- ESLint：通过
- npm 生产依赖审计：0 个已知漏洞
- Python 依赖一致性检查：通过
